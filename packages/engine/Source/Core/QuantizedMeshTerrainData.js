 ...
 * @param {Uint16Array|Uint32Array} options.indices 指定量化顶点如何链接
 *                      在一起形成三角形的索引。每三个索引指定一个三角形。
 * @param {number} options.minimumHeight 瓦片内的最小地形高度，以椭球体以上的米为单位。
 * @param {number} options.maximumHeight 瓦片内的最大地形高度，以椭球体以上的米为单位。
 * @param {BoundingSphere} options.boundingSphere 包围网格中所有顶点的球体。
 * @param {OrientedBoundingBox} [options.orientedBoundingBox] 包围网格中所有顶点的定向边界框。
 * @param {Cartesian3} options.horizonOcclusionPoint 网格的地平线遮挡点。如果此点
 *                      低于地平线，则假定整个瓦片也低于地平线。
 *                      该点以椭球体缩放坐标表示。
 ...
 * @param {number} options.westSkirtHeight 在瓦片西边缘添加的裙边高度。
 * @param {number} options.southSkirtHeight 在瓦片南边缘添加的裙边高度。
 * @param {number} options.eastSkirtHeight 在瓦片东边缘添加的裙边高度。
 * @param {number} options.northSkirtHeight 在瓦片北边缘添加的裙边高度。
 * @param {number} [options.childTileMask=15] 位掩码，指示此瓦片的四个子瓦片中哪些存在。
 *                 如果设置了子瓦片的位，则在需要时也会请求该瓦片的几何数据。如果清除了该位，
 *                 则不会请求子瓦片，而是从父瓦片向上采样几何数据。位值如下：
 *                 <table>
 ...
 *                  否则为false。
 * @param {Uint8Array} [options.encodedNormals] 包含每个顶点法线的缓冲区，使用'oct'编码进行编码
 * @param {Uint8Array} [options.waterMask] 包含水掩码的缓冲区。
 * @param {Credit[]} [options.credits] 此瓦片的信用声明数组。
 *
 *
 * @example
 ...
 * @returns {Promise<QuantizedMeshTerrainData>|undefined} 子瓦片的上采样高度图地形数据的Promise，
 ...
 * @returns {number} 指定位置的地形高度。位置被限制到
...
/**
 * 根据{@link QuantizedMeshTerrainData.childTileMask}确定给定的子瓦片是否可用。给定的子瓦片坐标假定
