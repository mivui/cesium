 ...
 * @property {number} [minimumLevel=0] 影像提供者支持的最小细节级别。指定此值时需注意，
 *                 最小级别的瓦片数量应较少，如四个或更少。较大的数值可能会导致
 *                 渲染问题。
 * @property {number} [maximumLevel] 影像提供者支持的最大细节级别，如果无限制则为 undefined。
 * @property {Rectangle} [rectangle=Rectangle.MAX_VALUE] 图像覆盖的矩形区域，单位为弧度。
 * @property {TilingScheme} [tilingScheme] 指定如何将椭球面
 * 分割为瓦片的瓦片方案。如果未提供此参数，则使用 {@link WebMercatorTilingScheme}。
 * @property {Ellipsoid} [ellipsoid] 椭球体。如果指定了 tilingScheme，
 *                    则忽略此参数，转而使用瓦片方案的椭球体。如果两个
 *                    参数都未指定，则使用 WGS84 椭球体。
 * @property {number} [tileWidth=256] 图像瓦片的像素宽度。
 * @property {number} [tileHeight=256] 图像瓦片的像素高度。
 * @property {boolean} [flipXY] 旧版本的 gdal2tiles.py 在 tilemapresource.xml 中翻转了 X 和 Y 值。
...
/**
 * <div class="notice">
 * 要构造 TileMapServiceImageryProvider，请调用 {@link TileMapServiceImageryProvider.fromUrl}。不要直接调用构造函数。
 * </div>
 *
 * 一个影像提供者，提供由
 * {@link http://www.maptiler.org/|MapTiler}、{@link http://www.klokan.cz/projects/gdal2tiles/|GDAL2Tiles} 等工具生成的瓦片影像。
 *
 ...
 *
 * @param {Resource|string} url 服务器上图像瓦片的路径。
 * @param {TileMapServiceImageryProvider.ConstructorOptions} [options] 描述初始化选项的对象。
 * @returns {Promise<TileMapServiceImageryProvider>} 一个解析为所创建的 TileMapServiceImageryProvider 的 Promise。
 *
 * @example
 * const tms = await Cesium.TileMapServiceImageryProvider.fromUrl(
 *    '../images/cesium_maptiler/Cesium_Logo_Color', {
 ...
 * });
 *
 * @exception {RuntimeError} 在 tilemapresource.xml 中无法找到预期的 tilesets 或 bbox 属性
 * @exception {RuntimeError} tilemapresource.xml 指定了不支持的 profile 属性
 */
