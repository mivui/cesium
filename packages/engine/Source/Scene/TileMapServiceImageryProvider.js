 ...
 * @property {TilingScheme} [tilingScheme] 指定椭球体如何
 ...
 * @property {boolean} [flipXY] 旧版本的 gdal2tiles.py 在 tilemapresource.xml 中翻转 X 和 Y 值。
 * @property {TileDiscardPolicy} [tileDiscardPolicy] 根据某些标准丢弃瓦片图像的策略
 * 指定此选项将执行相同的操作，允许加载这些不正确的图块集。
 */
 ...
 *
 * @param {Resource|String} url 服务器上图像图块的路径。
 * @param {TileMapServiceImageryProvider.ConstructorOptions} [options] 描述初始化选项的对象.
 * @returns {Promise<TileMapServiceImageryProvider>} 一个 Promise 解析为创建的 TileMapServiceImageryProvider。
 *
 * @example
 * const tms = await Cesium.TileMapServiceImageryProvider.fromUrl(
 *    '../images/cesium_maptiler/Cesium_Logo_Color', {
