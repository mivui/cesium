 ...
 * @returns {number} 交点作为沿射线的参数化距离，如果没有交点，则为 undefined。
 */
 ...
 * @param {Interval} [result] 存储结果的结果。
 * @returns {Interval} 包含沿射线的标量点的区间，如果没有交集，则为 undefined。
 */
 ...
 * 计算线段和平面的交集。
 *
 * @param {Cartesian3} endPoint0 线段的终点。
 ...
 * @param {Cartesian3} p1 三角形的第二点
 * @param {Cartesian3} p2 三角形的第三个点
 * @param {Plane} plane  相交平面
 * @returns {object} 具有 <code>positions</code> 和 <code>indices</code> 属性的对象，它们是表示不穿过平面的三个三角形的数组。（如果不存在交集，则为 Undefined）
 *
 * @example
 * const origin = Cesium.Cartesian3.fromDegrees(-75.59777, 40.03883);
 * const normal = ellipsoid.geodeticSurfaceNormal(origin);
